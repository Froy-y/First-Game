class Obstacle {
    constructor(x, y, width, height, speed, type){
        ...
        this.frameX = 0;
        this.frameY = 0;
        this.randomise = Math.floor(Math.random() * 30 + 30); //uses math.floor bc doesnt want decimals
    ...
    update(){
        this.x += this.speed * gameSpeed; //has to be multi not addition because it can come out negative sometimes
        if (this.speed > 0){
...
function initObstacles(){
    ...
    for (let i = 0; i < 2; i++){
        ...
        ); //carsArray was originally an empty arry
    } // essentially this is defining constructor(x, y, width, height, speed, type) to these vaules ^^^^^^
...
initObstacles(); //do this to fill array with obstacle objects for lane 1
